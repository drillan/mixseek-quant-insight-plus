...
# mixseek-quant-insight-plus Constitution
...
### Article 3: CLI & Agent Design Compliance（CLI・Agent設計準拠）

すべてのCLI・Agent実装は、設計原則に準拠しなければならない（MUST）。
...
**Agent要件**:
- エージェントは独立してテスト可能でなければならない（MUST）
- 設定はTOMLファイルで一元管理しなければならない（MUST）
- MemberAgentFactoryへの登録はパッケージ初期化時に自動実行されなければ
  ならない（MUST）

**理由**: CLI/Agentの一貫した使用体験と保守性を保証
...
- CLI/Agent設計原則に準拠しているか（Article 3）
